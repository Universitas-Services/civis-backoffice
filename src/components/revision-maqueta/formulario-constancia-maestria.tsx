...
import {
  KEY_PREFIJO_CEDULA_POSTULANTE_TESIS_MAESTRIA,
...
export function FormularioConstanciaMaestria({
  valores,
  errores = {},
  onCampo,
}: {
  ...
  return (
    <div className="space-y-4">
      <fieldset className="space-y-3">
        ...
        <CampoTexto
          id="nombre_universidad_tesis_maestria"
          etiqueta="Nombre completo de la casa de estudios superiores"
          ayuda="Ejemplo: Universidad Central de Venezuela"
          value={String(valores.nombre_universidad_tesis_maestria ?? "")}
          error={errores.nombre_universidad_tesis_maestria}
          onChange={(v) => onCampo("nombre_universidad_tesis_maestria", v)}
        />

        <CampoTexto
          id="area_maestria_tesis"
          etiqueta="Área o especialidad exacta de la maestría"
          ayuda="Ejemplo: Maestría en Derecho Constitucional"
          value={String(valores.area_maestria_tesis ?? "")}
          error={errores.area_maestria_tesis}
          onChange={(v) => onCampo("area_maestria_tesis", v)}
        />

        <CampoArea
          id="titulo_trabajo_maestria"
          etiqueta="Título del trabajo especial de grado / tesis de maestría"
          ayuda='Ejemplo: "Límites político-criminales y dogmáticos de la autoría mediata…"'
          value={String(valores.titulo_trabajo_maestria ?? "")}
          error={errores.titulo_trabajo_maestria}
          onChange={(v) => onCampo("titulo_trabajo_maestria", v)}
        />
        ...
        <CampoTexto
          id="nombre_postulante_tesis_maestria"
          etiqueta="Nombre completo del profesional"
          ayuda="Coloca el nombre del profesional que presenta el trabajo"
          value={String(valores.nombre_postulante_tesis_maestria ?? "")}
          error={errores.nombre_postulante_tesis_maestria}
          onChange={(v) => onCampo("nombre_postulante_tesis_maestria", v)}
        />

        <CampoTexto
          id="apellido_postulante_tesis_maestria"
          etiqueta="Apellido completo del profesional"
          ayuda="Coloca el Apellido del profesional que presenta el trabajo"
          value={String(valores.apellido_postulante_tesis_maestria ?? "")}
          error={errores.apellido_postulante_tesis_maestria}
          onChange={(v) => onCampo("apellido_postulante_tesis_maestria", v)}
        />

        <CampoCedulaVe
          id="cedula_postulante_tesis_maestria"
          ...
          prefijoKey={KEY_PREFIJO_CEDULA_POSTULANTE_TESIS_MAESTRIA}
          prefijo={String(valores[KEY_PREFIJO_CEDULA_POSTULANTE_TESIS_MAESTRIA] ?? "V")}
          digitos={String(valores.cedula_postulante_tesis_maestria ?? "")}
          error={errores.cedula_postulante_tesis_maestria}
          onCampo={onCampo}
        />
        ...
        <CampoArea
          ...
          value={String(valores.jurado_examinador_maestria ?? "")}
          error={errores.jurado_examinador_maestria}
          onChange={(v) => onCampo("jurado_examinador_maestria", v)}
        />

        <CampoTexto
          id="veredicto_calificacion_maestria"
          etiqueta="Veredicto / Calificación"
          ayuda="Ejemplo: Aprobada por unanimidad, publicación, mención Honorífica, o nota numérica"
          value={String(valores.veredicto_calificacion_maestria ?? "")}
          error={errores.veredicto_calificacion_maestria}
          onChange={(v) => onCampo("veredicto_calificacion_maestria", v)}
        />
        ...
        <div>
          ...
          <DatePicker
            id="fecha_defensa_maestria"
            value={String(valores.fecha_defensa_maestria ?? "")}
            onChange={(iso) => onCampo("fecha_defensa_maestria", iso)}
            toYear={new Date().getFullYear()}
            invalid={Boolean(errores.fecha_defensa_maestria)}
          />
          {errores.fecha_defensa_maestria && (
            <p className="mensaje-error-campo">{errores.fecha_defensa_maestria}</p>
          )}
        </div>
      </fieldset>
    </div>
