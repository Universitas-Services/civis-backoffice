...
import type { PostulanteRevisionLista } from "@/lib/adaptar-revision-api";
...
export function ListaPostulantesRevision({
  ...
  readonly items: readonly PostulanteRevisionLista[];
}) {
  return (
        ...
        <table className="w-full text-center text-sm">
          ...
          <thead className="border-b-2 border-toga-300 bg-toga-50">
            <tr>
              ...
              <th scope="col" className="px-4 py-3 font-semibold text-toga-700">
                <ConTooltip texto="Documentos cargados en el expediente">
                  <button type="button" className="cursor-help font-semibold">
                    Docs.
                  </button>
                </ConTooltip>
            ...
            {items.map((p) => (
              <tr key={p.id} className="hover:bg-toga-50">
                ...
                <td className="px-4 py-3">
                  ...
                  <p className="codigo mt-0.5 text-xs text-toga-500">{p.cedula}</p>
                </td>
                ...
                <td className="cifra px-4 py-3 text-toga-600">
                  <ConTooltip
                    texto={`${p.docsTotal} documento${p.docsTotal === 1 ? "" : "s"} cargado${p.docsTotal === 1 ? "" : "s"}`}
                  >
                    <span className="cursor-default font-medium text-toga-800">{p.docsTotal}</span>
                  </ConTooltip>
                </td>
                <td className="px-4 py-3">
          ...
          <li key={p.id}>
            ...
            >
              ...
              <p className="mt-3 text-xs text-toga-500">
                Documentos:{" "}
                <span className="cifra font-medium text-toga-800">{p.docsTotal}</span>
              </p>
            </Link>
          </li>
        ))}
